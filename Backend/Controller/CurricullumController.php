...
//Profile Picture---------------------------------------- 
function render_curricullum_picture($id,$errormessage,$renderpath)
{
   $datas=$this->getcurricullumbyid($id);
    foreach($datas as $data)
    {
    ...
    $updateurl = str_replace(':id',$id,$this->app->urlFor('updateprofilepicture'));
    $listurl =  str_replace(':id', $id,$this->app->urlFor('editcurricullum'));
...
function update_picture_item($username,$id,$files,$renderpath)
{
    ...
    foreach($datas as $data)
    {
        $oldpicture=$data["profilepicture"];
        $cvname =$this->quit_special_chars($data["name"]);
    }  
...
function update_picture($username,$id,$profilepicture)
{
   $dt = date('Y-m-d H:i:s');
        $this->database->update("curricullum",
            array(
